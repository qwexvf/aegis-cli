// Package rbscan implements astscan.LanguageScanner for Ruby using
// tree-sitter with the tree-sitter-ruby grammar. Detection patterns
// mirror the Python scanner; see pyscan/queries.scm for the design
// rationale and capabilityFor() for the capture→Capability mapping.
//
// Coverage today (mirrors pyscan capability set):
//
