...
func TestYarn_ParseInstallArgs(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			want: []SpecToken{{Name: "ua-parser-js", Version: "0.7.29", Raw: "ua-parser-js@0.7.29"}},
		},
		{
		...
		},
		{
			...
			want: []SpecToken{{Raw: "portal:./pkg", NonRegistry: true}},
		},
		{
			name: "berry patch protocol",
			argv: []string{"add", "patch:lodash@4.17.21#./fix.patch"},
			want: []SpecToken{{Raw: "patch:lodash@4.17.21#./fix.patch", NonRegistry: true}},
		},
		{
			name: "berry npm: protocol",
			argv: []string{"add", "npm:@types/lodash@^4"},
			want: []SpecToken{{Raw: "npm:@types/lodash@^4", NonRegistry: true}},
		},
		{
