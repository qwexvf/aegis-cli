...
// DetectTarballDriftFromSources runs the pure drift.Diff over
...
func DetectTarballDriftFromSources(
	...
	repoFiles []string,
	repoSubdir string,
) (domain.Capability, []drift.DriftEvidence) {
	...
	ev := drift.Diff(drift.DiffInputs{
...
func hasScriptFileEvidence(ev []drift.DriftEvidence) bool {
	for _, e := range ev {
...
func isLikelyMonorepoSubdirMismatch(ev []drift.DriftEvidence, tarballPaths []string) bool {
