...
// chain compromises against the ruby AST scanner. Companion to the
...
// scanner via DetectSourcePatterns instead of ruby.
...
func scanWithEvidence(t *testing.T, path, src string) *ast.Findings {
	t.Helper()
	s, err := New()
	if err != nil {
		t.Fatalf("New: %v", err)
	}
	f := ast.NewFindingsWithEvidence()
...
func requireCaps(t *testing.T, f *ast.Findings, want ...domain.Capability) {
	t.Helper()
	for _, c := range want {
...
func requireEvidence(t *testing.T, f *ast.Findings, c domain.Capability, snippetSubstr string) {
	t.Helper()
...
func TestIncidents_GemspecPostInstallShellOut(t *testing.T) {
	...
	// ast.isAnalyzable now routes .gemspec through ruby, so
