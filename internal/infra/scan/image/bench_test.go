...
func BenchmarkOverlayLayers_Tiny(b *testing.B) {
	...
	for b.Loop() {
		_, _, _, _, _, _ = overlayLayersFull([]v1Layer{&fakeLayer{entries: tarballToEntries(tarball)}}, ScanOpts{}, defaultLockfileNames())
	}
}
...
func BenchmarkOverlayLayers_1000Files(b *testing.B) {
	...
	b.ReportAllocs()
	for b.Loop() {
		_, _, _, _, _, _ = overlayLayersFull([]v1Layer{&fakeLayer{entries: tarballToEntries(tarball)}}, ScanOpts{CapturePackageSources: true}, defaultLockfileNames())
	}
}
