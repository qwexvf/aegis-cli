...
> Entries at `0.29.1` and below describe the **Go** implementation, frozen on the
...
* **BREAKING — Go v0.29 is not a drop-in.** The Rust CLI is flat, stateless by
  default, and CI-first. Scanning behavior is at parity, the command surface is
  not. See the migration table in [README.md](README.md#migrating-from-go-v029).
* **BREAKING — `analyze`** scans a local directory (Go's `analyze --local`).
  Fetch-and-scan by package spec moved to `explain <name@version>`.
* **BREAKING — `actions`** *generates* a GitHub Actions workflow. Go's
  `actions scan` (scanning workflows for risk) is not ported.
* **BREAKING — `allowlist`** is a read-only dump of the built-in rules.
  `add`/`remove`/`test`/`verify`/`sync` are not ported.
* **BREAKING — `ci --fail-on`** is a *verdict* threshold (`safe`/`review`/
  `prompt`/`block`, default `block`), not a severity.
* **Not ported (cloud-coupled):** `cloud`, `admin`, `recheck`, `snapshot submit`,
  `allowlist sync`. **Not ported (local):** `cache`, `audit`, `doctor`,
  `completion`, `hook uninstall`, interactive TUI.

### Added

* **Rust-ahead of Go v0.29:** `ci` enrichment fetches and scans published source
  for all five supported ecosystems (npm/PyPI/crates/RubyGems/Go — Go did npm
  only); reachability downgrade extends to PyPI/Go/RubyGems; `sbom` extracts npm
  `lockfileVersion` 3 transitive edges into CycloneDX `dependsOn`.
* **New commands with no Go analog:** `parse`, `run` (an `aegis.toml` parallel
  task runner), `reach` (standalone reachability), `snapshot capture`
  (single-package capability fingerprint + `--baseline` drift), and `explain`'s
  capability-catalog mode.
* **`snapshot` lifecycle** ported: `save` / `show` / `diff` / `enrich` /
  `verify` / `rescan`. `enrich` is idempotent — a resumed run only processes
  deps that still lack a fingerprint. `rescan` exits 1 when a new advisory
  appears, for cron use.
* **Lean builds.** Every lockfile ecosystem, tree-sitter grammar, and heavy
  heuristic sits behind a Cargo feature; `--no-default-features --features
  npm,pypi,rust` strips the rest. A CI job guards against leaked dependencies.

### Fixed

Bugs found by diffing the Rust implementation against Go and fixed in both
directions during the port:

* `install-hook` base flag was not populated
* R and Perl obfuscated-payload regexes did not match
* `.ex`/`.exs` were missing from analyzable-source detection (Elixir/hex)
* the Java tree-sitter grammar was absent, so Maven fixtures under-reported
* advisory alias-over-dedup collapsed OSV mutual alias pairs (10 advisories
  reported as 8), which also corrupted `fix` and SARIF output
* `ci --json` omitted advisory `fixed_in` and `in_kev`
* CycloneDX output omitted `$schema` and emitted `dependsOn: []` where Go omits
  the key
* prerelease version ordering ranked `1.2.3-rc.1` above `1.2.3`, so a prerelease
  `fixed_in` could look like a valid forward upgrade

### Internal

* No CGO and no async runtime. 13 native tree-sitter grammars; blocking HTTP via
  `ureq` behind an `HttpClient` trait; rayon for parallelism.
* Three parity harnesses run as blocking CI against goldens captured from Go
  v0.29 — `analyze` 36/36 across 13 ecosystems, `sbom` 4/4, `ci` 6/6. All
  offline; `ci-parity` replays a committed HTTP cassette.
* 549 tests, including an end-to-end suite that reconstructs real supply-chain
  attacks and drives the built binary.
...
* **ci:** supply-chain self-audit — aegis scans its own dependencies on every PR/push; daily retroactive CVE rescan
* **security:** all GitHub Actions `uses:` pinned to commit SHAs across all workflow files


### Fixed

* `pubspec.lock` parser double-flushed last package when `sdks:` section present
* `sbom validate` CI job missing `snapshot save` before `aegis sbom`
